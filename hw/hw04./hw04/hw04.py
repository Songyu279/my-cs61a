def shuffle(s):
    ...
    return [s[:len(s) // 2][i // 2] if i % 2 == 0 else s[len(s) // 2 :][i // 2]
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    return (total_mass(end(left(m)))) * length(left(m)) == total_mass(right(m)) and balanced(end(left(m))) and balanced(end(right(m)))
